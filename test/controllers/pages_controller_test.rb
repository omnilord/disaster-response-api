...
class PagesControllerTest < DevisedTest
  ...
  test 'Loading index anonymous creates default page' do
  ...
  test 'Loading index as a generic user creates default page' do
    assert_difference('Draft.count', 1) do
      assert_difference('Page.count', 1) do
  ...
  test 'Loading index as an admin user creates default page' do
    assert_difference('Draft.count', 1) do
      assert_difference('Page.count', 1) do
        sign_in users(:admin)
        get root_path
  ...
  test 'New pages contain fields as submitted' do
    ...
    assert_nil draft.denied_by
    assert_equal users(:admin), page.editor
  ...
  test 'Admin users create and apply a draft when updating pages' do
    ...
    assert_difference('Draft.count', 1) do
      assert_difference('Page.count', 0) do
        assert_changes(-> { { page: page.page, title: page.title, content: page.content } },
                       from: { page: page.page, title: page.title, content: page.content },
                       to: params[:page]) do
          patch page_path(page), params: params
          assert_response :redirect
          page.reload
